...
input_file = r"C:\Users\HP\Desktop\hashes\leftfrom35k.xlsx"
output_file =r"C:\Users\HP\Desktop\hashes\leftfrom35k.csv"


# ✅ Load all hashes (SHA1 and SHA256)
df = pd.read_excel(input_file)
hash_list = df.iloc[:, [0, 1]].values.tolist()  # Only 2 columns: SHA1, SHA256

# ✅ Load processed hashes from output CSV if exists
if os.path.exists(output_file):
    existing_df = pd.read_csv(output_file)
    existing_df = existing_df.drop_duplicates(subset='SHA1')
    existing_df.to_csv(output_file, index=False)
    processed_hashes = set(existing_df['SHA1'].dropna())
else:
    processed_hashes = set()

# ✅ Resume progress
last_processed_index = -1
for idx, (sha1, sha256) in enumerate(hash_list):
    if sha1 in processed_hashes:
...
def get_virustotal_data(sha1=None, sha256=None):
    headers = {"x-apikey": API_KEY}
    for hash_val in [sha1, sha256]:
    ...
    print(f"No data found for: {sha1}")
    return [sha1, 'Error', 'Error', 'Error']

results = []
processed_count = 0

for i in range(start_index, len(hash_list)):
    sha1, sha256 = hash_list[i]

    if sha1 in processed_hashes:
        continue

    print(f"Processing {sha1} ({i+1}/{len(hash_list)})")
    result = get_virustotal_data(sha1, sha256)
    results.append(result)
    processed_hashes.add(sha1)
    processed_count += 1

    # ✅ Save every 10 results
    if len(results) >= 10:
        temp_df = pd.DataFrame(results, columns=['SHA1', 'MalScore', 'Creation Time', 'Last Submission'])
        if os.path.exists(output_file):
            combined = pd.concat([pd.read_csv(output_file), temp_df]).drop_duplicates(subset='SHA1')
        else:
            combined = temp_df
